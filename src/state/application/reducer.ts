  ...
  builder
    ...
    .addCase(toggleStatusModal, (state, action) => {
      state.statusModalOpen =
        action.payload.statusModalOpen !== undefined
          ? action.payload.statusModalOpen
          : !state.statusModalOpen;
    })
    .addCase(toggleStatus, (state, action) => {
      state.status =
        action.payload.status !== undefined ? action.payload.status : 1;
    })
    .addCase(updateTransactionHash, (state, action) => {
      state.transactionHash =
        action.payload.transactionHash !== undefined
